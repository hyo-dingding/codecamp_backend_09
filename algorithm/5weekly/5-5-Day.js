...
function solution(N, stages) {
    ...
    for (let i = 1; i <= N; i++) {
        // console.log(i ) // 스테이지로 활용
        failArr.push({
            stages: i, // 스테이지번호
            users: 0, // 클리어하지 못한 유저수
            fail: 0, // 실패율저장
        });
    }
    // console.log(failArr)
    let allUsers = stages.length; // 총유저의 수를 저장
    for (let i = 0; i < stages.length; i++) {
        ...
        if (failArr[stages[i] - 1] !== undefined) {
            ...
            // 현재스테이지의 번호와 다음 스테이지 번호가 다를때
            // 현재 스테이지의 정보 참조가 끝났을때
            if (stages[i] !== stages[i + 1]) {
                ...
                // console.log(failArr)  // 실패율기준으로 내림차순
            }
        }
    }
